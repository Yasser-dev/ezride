...
export const locationReachedNotification = functions.https.onRequest(
  ...
  .onUpdate(async (snapshot) => {
    ...
    if (rideRequest.status === "accepted") {
      ...
      users.forEach((document) => {
        ...
        console.log(`another user id: ${rideRequest.userId}`);

        if (userData.id === rideRequest.userId) {
      ...
      const payload: admin.messaging.MessagingPayload = {
        ...
        data: {
          ...
          distance_value: rideRequest.distance.value.toString(),
          destination_latitude: rideRequest.destination.latitude.toString(),
          destination_longitude: rideRequest.destination.longitude.toString(),
          id: rideRequest.id,
      ...
      console.log("RIDE STATUS IS: " + rideRequest.status);
      return;
    }
  });
